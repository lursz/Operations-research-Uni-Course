...
class ImplicitEnumerationSolver(IntegerProgrammingSolver):
    ...
    def _branch_and_bound(self, partial_assignment: Dict[sseexp.Variable, int], left: Set[sseexp.Variable]) -> None:
        ...
        najlepsze_przypisanie = self._best_possible_assignment(partial_assignment)

        ograniczenie_gorne = self._enum_model.objective.evaluate(najlepsze_przypisanie)

        if ograniczenie_gorne < self._lower_bound():
            return None
        
        if not self._is_model_satisfiable_with_partial_assignment(partial_assignment):
            return None
        
        if self._total_infeasibility(najlepsze_przypisanie) == 0:
            if self.best_solution is None or self.best_solution.objective_value() < ograniczenie_gorne:
                self.best_solution = Solution.with_assignment(self.model, najlepsze_przypisanie, True)
            return None
        ...
        zmienna_do_branchowania = self._select_var_to_branch(left, partial_assignment)

        partial_assignment[zmienna_do_branchowania] = 0
        left.remove(zmienna_do_branchowania)
        self._branch_and_bound(partial_assignment, left)
        partial_assignment[zmienna_do_branchowania] = 1
        self._branch_and_bound(partial_assignment, left)
        del partial_assignment[zmienna_do_branchowania]
        left.add(zmienna_do_branchowania)
    ...
    def _best_possible_assignment(self, partial_assignment: Dict[sseexp.Variable, int]) -> List[int]:
        ...
        return [0 if var not in partial_assignment else partial_assignment[var] for var in self._enum_model.variables]
    ...
    def _is_model_satisfiable_with_partial_assignment(self, partial_assignment: Dict[sseexp.Variable, int]) -> bool:
        ...
        for warunek in self._enum_model.constraints:
            wynik = 0
            wspolczynniki = warunek.expression.coefficients(self._enum_model)
            for var in self._enum_model.variables:
                if var in partial_assignment:
                    wynik += wspolczynniki[var.index] * partial_assignment[var]
                elif wspolczynniki[var.index] < 0:
                    wynik += wspolczynniki[var.index]
            if wynik > warunek.bound:
                return False
    ...
    def _select_var_to_branch(self, left: Set[sseexp.Variable], partial_assignment: Dict[sseexp.Variable,
        ...
        for zmienna in left:
            partial_assignment[zmienna] = 1
            wartosc = self._total_infeasibility(self._best_possible_assignment(partial_assignment))
            if najlepsza is None or wartosc < najlepsza_wartosc or (wartosc == najlepsza_wartosc and zmienna.index < najlepsza.index):
                najlepsza_wartosc = wartosc
                najlepsza = zmienna
            del partial_assignment[zmienna]
    ...
    def _total_infeasibility(self, assignment: List[int]) -> int:
        ...
        #       and check how much its value surpasses the bound.
        #       Then add the result to the total_infeasibility
        #
        # [1] constraint.expression.evaluate(<assignment>)

        for warunek in self._enum_model.constraints:
            total_infeasibility += max(0, warunek.expression.evaluate(assignment) - warunek.bound)
