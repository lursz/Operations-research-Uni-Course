...
class GreedySolver(Solver):
    ...
    def _solve_using_heuristic(self, heuristic: Heuristic) -> Solution:
        ...
        posortowane = sorted(self.problem.items, key = heuristic, reverse = True)

        zwracane = []
        suma_wag, suma_wart = 0, 0
        for przedmiot in posortowane:
            if suma_wag + przedmiot.weight <= self.problem.capacity:
                suma_wag += przedmiot.weight
                suma_wart += przedmiot.value
                zwracane.append(przedmiot)
        
        return Solution(zwracane, suma_wart, suma_wag, False)
